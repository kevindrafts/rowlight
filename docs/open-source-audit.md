...
- The maintainer explicitly approved disclosure of the retained history, legacy preview downloads, historical machine paths, and commit author metadata. The repository is now public at `kevindrafts/rowlight`. GitHub secret scanning, push protection, and private vulnerability reporting are enabled. The earlier private-repository plan restriction no longer applies; no paid upgrade was performed.
